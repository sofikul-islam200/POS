<template>
    ...
    <div class="container">
        <div class="card pd-20 pd-sm-40 mt-4" id="cards" style="box-shadow: 0 3px 6px rgba(0, 0, 0, 0.16), 0 3px 6px rgba(0, 0, 0, 0.23);border-radius: 0px;">
            <h6 class="card-body-title">Update Single Customer</h6><br/>
...
export default {
    ...
    created(){
        ...
        console.log(id);
        axios.get('/api/GetSingleCustomerEdit/'+id)
    ...
    methods:{
       Image(event){
           ...
           if(file.size > 1048770){
            iziToast.error({
                title: 'error',
                message: 'image size less then 1 mb',
                position:'topRight',
                timeout:2000,
            });
           }else{
       ...
       updateCustomer(){
           ...
           axios.post('/api/UpdateCustomer/' +id,this.form)
           .then(() =>{
                iziToast.success({
                title: 'success',
